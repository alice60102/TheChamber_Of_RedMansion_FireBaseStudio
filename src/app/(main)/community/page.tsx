...
import { Users, MessageSquare, PlusCircle, ThumbsUp, MessageCircle, Search } from "lucide-react";
...
const initialPosts = [
  {
    id: '1',
    author: '寶玉哥哥',
    avatar: 'https://placehold.co/40x40.png?text=寶',
    timestamp: '2小時前',
    ...
    comments: 4,
    tags: ['判詞解析', '人物探討'],
    image: 'https://placehold.co/600x400.png?text=判詞討論',
    imageHint: 'ancient scroll'
  },
  {
    id: '2',
    author: '瀟湘妃子',
    avatar: 'https://placehold.co/40x40.png?text=黛',
    timestamp: '5小時前',
    title: '葬花詞中的意境與情感',
  ...
  },
  {
    id: '3',
    author: '村夫賈雨村',
    avatar: 'https://placehold.co/40x40.png?text=雨',
    timestamp: '1天前',
    ...
    comments: 2,
    tags: ['社會背景', '歷史研究'],
    image: 'https://placehold.co/600x400.png?text=清代生活',
    imageHint: 'chinese street'
  },
];
        ...
        <CardHeader>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <Button className="bg-accent text-accent-foreground hover:bg-accent/90">
              <PlusCircle className="h-4 w-4 mr-2" /> 創建新帖子
            </Button>
          </div>
        </CardHeader>
        <CardContent>
              ...
              {filteredPosts.map((post) => (
                <Card key={post.id} className="shadow-lg overflow-hidden bg-card/70 hover:shadow-primary/10 transition-shadow">
                  ...
                  <CardHeader>
                    ...
                    <CardTitle className="font-artistic text-xl">{post.title}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-foreground/80 leading-relaxed line-clamp-3">{post.content}</p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {post.tags.map(tag => (
                        <span key={tag} className="text-xs bg-accent/20 text-accent-foreground py-0.5 px-2 rounded-full">#{tag}</span>
                      ))}
                    </div>
                  </CardContent>
                  <CardFooter className="flex justify-between items-center pt-4 border-t border-border/50">
                    ...
                    <Button variant="outline" size="sm">閱讀全文</Button>
                  </CardFooter>
                </Card>
              ))}
            </div>
