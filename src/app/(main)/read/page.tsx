...
export default function ReadPage() {
  ...
  const handleMouseUp = useCallback((event: globalThis.MouseEvent) => {
    ...
    // If clicking inside a sheet or on selection action buttons, do nothing to the selection and handle toolbar.
    ...
    // If clicking on an area explicitly marked to not affect selection (like toolbar), clear selection.
    ...
    if (text.length > 0 && chapterContentRef.current && selection && selection.rangeCount > 0) {
      ...
      if (chapterContentRef.current.contains(range.commonAncestorContainer)) {
        ...
        const scrollArea = document.getElementById('chapter-content-scroll-area'); // Assuming your ScrollArea has this ID
        
        // Calculate scroll offsets. If in a scrollable container, use its scrollTop/Left. Otherwise, use window scroll.
        ...
        const top = rect.bottom + scrollTop + 5; // Position below selection
        ...
        // Selection is outside the readable content area
        setSelectedTextInfo(null);
      }
    } else {
      // No text selected or selection is invalid
      setSelectedTextInfo(null);
    }
    // Always call handleInteraction after a short delay to ensure state updates for selection are processed first.
    setTimeout(() => handleInteraction(), 0);
  }, [handleInteraction]);


  useEffect(() => {
    // Listen for mouse up on the document to handle text selection
    document.addEventListener('mouseup', handleMouseUp);
    // Listen for scroll and mouse move on the document to show toolbar
  ...
  const handleOpenNoteSheet = () => {
    if (selectedTextInfo?.text) {
      setCurrentNote(""); // Optionally pre-fill with selectedTextInfo.text or keep it blank
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div
        ...
        onClick={(e) => e.stopPropagation()} // Prevent clicks on toolbar from clearing selection via document listener
      >
      ...
      <Sheet open={isNoteSheetOpen} onOpenChange={setIsNoteSheetOpen}>
        ...
        >
          ...
          <SheetFooter className="p-4 border-t border-border flex justify-between">
            <SheetClose asChild>
              <Button variant="outline" onClick={() => {setIsNoteSheetOpen(false); handleInteraction();}}>取消</Button>
            </SheetClose>
            <Button
              onClick={() => {
      ...
      <Sheet open={isAiSheetOpen} onOpenChange={setIsAiSheetOpen}>
        ...
        >
            ...
            <SheetFooter className="p-4 border-t border-border">
                 <SheetClose asChild>
                    <Button variant="outline" onClick={() => {setIsAiSheetOpen(false); handleInteraction();}}>關閉</Button>
                 </SheetClose>
            </SheetFooter>
        </SheetContent>
      </Sheet>
